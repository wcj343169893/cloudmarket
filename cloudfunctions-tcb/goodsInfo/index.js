...
exports.main = async (event, context) => {
	...
	if (uid > 0) {
		...
		for (let item of goods.data) {
			...
			//增加浏览记录,必须存在uid
			let visite = await db.collection('goods_visites').where({
				uid: uid,
				goods_id: item.id
			}).field({
				goods_id: 1,
			...
			if (visite.data.length < 1) {
				//新增浏览记录
				let visave = await db.collection('goods_visites').add({
					uid: uid,
					goods_id: item.id,
					title: item.title,
					shopid: item.shopid,
				...
				//增加浏览量
				let incResult = await collection.doc(item._id).update({
					visite: cmd.inc(1)
				});
				...
				//更新浏览时间
				let updateVisite = await db.collection('goods_visites').doc(visite.data[0]._id).update({
					src: item.src,
					title: item.title,
					price: item.price,
					time: time
