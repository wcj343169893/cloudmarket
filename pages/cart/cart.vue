								...
								<view class="desc-name">
									...
									<view class="weui-flex desc-content">
										...
										<uni-number-box
											...
											:max="item.limit"
											:stock="item.stock"
											:keys="item.goods_id + '_' + item.sku_id"
											:value="item.cart"
											@change="bindChangeCart"
										></uni-number-box>
									</view>
								</view>
							</view>
								...
								<view class="desc">
									<view class="image-wrapper" @click="navToGoodsPage(item)">
										<image
											...
											mode="aspectFill"
											lazy-load
											@load="onImageLoad('notEnoughCartList', index)"
											@error="onImageError('notEnoughCartList', index)"
										></image>
									</view>
									<view class="desc-name">
										<view class="" @click="navToGoodsPage(item)">
...
import { editCart, cleanCart, selectCart, getCartList,settlementCart } from '@/common/request.js';
import { updateCartNumber, navToGoodsItemPage, navToLoginPage, navToCreateOrder, updateGoodsTags } from '@/common/functions.js';
export default {
	data() {
		return {
			isLoaded: false,
	...
	methods: {
					...
					res.goods.map(ele => {
						...
						if (ele.hasSku) {
							ele.skus.map(sku => {
								let key = id + '_' + sku.id;
								let item = { ...ele };
								goodsSkusMap[key] = Object.assign(item, sku, {
									key: key,
									goods_id: ele.id,
									sku_id: sku.id,
		...
		updateCartDataList(id, sid, numb) {
			...
			if (numb < 1) {
				...
				this.cartList[index].cart = numb;
				//判断库存
				/* if(this.cartList[index].cart > this.cartList[index].stock){
